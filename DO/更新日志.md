...
### [13.10.1](https://resource.dopus.com/t/directory-opus-13-10-1-beta/52545)
文件窗口：
...
文件列表：
- “相对大小”列现在在 “此电脑” 文件夹（Opus 原生实现）中起作用。
- 缩略图样式列表（配置 / 文件列表模式 / 缩略图 / 样式）现在可以重新排序，这允许控制样式的相对优先级。

文件操作：
- Opus 现在尊重 `NoCDBurning` 系统策略。

命令：
- 添加了 `Close RESTART` 命令。关闭 Opus 并重新启动它（立即重启，或者提供 `confirm` 参数来进行确认）。
...
求值器：
- 添加了 `FindWindow()` 求值器函数。允许通过类和标题搜索顶级窗口。如果指定的窗口存在，则返回 `true`。

脚本：
- 在脚本对话框中，`Control.AddItem` 和 `InsertItemAt` 现在允许在将 `DialogListItem` 添加到另一个 listview 控件时指定一个组 ID。以前，只有传递字符串和数据值才能为新项目指定组。

修复：
- 修复了单次点击模式和文件列表背景双击事件之间的交互，这可能会导致在双击向上移动后自动进入一个新文件夹。
- 修复了 “在文件名上开始套索” 选项阻止 ctrl+click 在详细信息模式下取消选中文件的问题。
- 从通过 shell 枚举生成的 `Go FOLDERCONTENT` 菜单中选择快捷方式现在可以正确运行快捷方式。

### [13.10.2](https://resource.dopus.com/t/directory-opus-13-10-2-beta/52746)
文件列表：
...
- 修复了偏好设置中的命令字段将 `&` 字符添加到通过其下拉菜单选择的命令名称的问题
...
### [13.9.5](https://resource.dopus.com/t/directory-opus-13-9-5-beta/52260)

### [13.9.6](https://resource.dopus.com/t/directory-opus-13-9-6-beta/52309)

### [13.9.7](https://resource.dopus.com/t/directory-opus-13-9-7-beta/52369)
...
### [13.9.8](https://resource.dopus.com/t/directory-opus-13-9-8-beta/52420)
