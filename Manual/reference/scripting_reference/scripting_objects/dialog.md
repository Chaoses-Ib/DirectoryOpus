...
| language | *string* | Set this property to create a [script dialog](/Manual/scripting/script_dialogs/RAEDME.md) in a particular language (if one or more [language overlays](/Manual/scripting/script_dialogs/dialog_editor/language_overlays.md) have been provided), rather than the currently selected language. |
...
Once the dialog has been created and its controls initialized, you should call **Show** or **RunDlg** to make it visible to the user. It will also go visible at the first **GetMsg** call if it hasn't already been shown. \$\$ Control \$\$ \<string:name\>  
\[\<string:dialog\>\]  
