**DOpusFactory** 对象是一个帮助对象，可以通过它创建各种其它对象。与表示已有 *事物*（例如 **[文件窗口](lister.zh.md)** 或 **[Tab](tab.zh.md)**）的对象不同，**DOpusFactory** 创建的对象是独立对象，可以在需要其功能时对其进行实例化。**DOpusFactory** 对象是通过 **[DOpus](dopus.zh.md).Create** 方法获得的。
