...
- @label -- 新修饰符，使用求值器代码动态更改按钮的标签。
  ...
  - //<示例://> `@label=viewmode=="details" ? "详细模式" : "其它模式"`
...
- `Favorites EXCLUDEBRANCH` -- 允许通配符在工具栏/菜单上显示它时排除收藏列表的分支，
  - 收藏栏的“其它收藏”菜单使用它来避免显示“收藏栏”分支的第二个副本，该副本显示在工具栏本身上。
...
- `Clipboard COPYIMAGE` -- 将所选图像文件作为位图数据复制到剪贴板，以便粘贴到其它程序中。
...
- 一条命令现在可以在编辑器中拆分到多行，论点在后续行中。
  ...
  - 对于普通“原始”/R 论点也很有用，通常它们会吞下命令行的其它部分（因此无法在同一个命令中使用两个）。
  - 要将论点移到自己单独的一行，语法要求…
    - 所有内容都位于一行中，用 \[\[ 和 \]\] 括起来。
    - 或者：`[[` 和 `]]` 位于不同的行中，其它所有内容位于它们之间。
...
下一篇：[其它脚本](/Manual/release_history/opus13_detailed/misc_scripting.zh.md)