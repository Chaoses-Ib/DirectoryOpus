...
- In the [filter control](/Manual/file_operations/filtered_operations/RAEDME.md) you can define a [textual filter](/Manual/file_operations/filtered_operations/textual_filters.md), and this uses the evaluator if it begins with \<ib:inline-code\>`=`\</ib:inline-code\>.
- Similarly, various commands let you define a textual filter from the command line (e.g. \<ib:inline-code\>`Find FILTERDEF`\</ib:inline-code\>), and these use the evaluator if they begin with \<ib:inline-code\>`=`\</ib:inline-code\>.
- If the [search field](/Manual/basic_concepts/searching_and_filtering/windows_search.md) is set to use Opus Find, you can enter an evaluation expression into it directly, beginning with a \<ib:inline-code\>`=`\</ib:inline-code\>.
...
The special variables \<ib:inline-code\>`subfolder`\</ib:inline-code\> and \<ib:inline-code\>`quick_filter`\</ib:inline-code\> tell you which mode the expression is being called for. Handling recurse mode (when \<ib:inline-code\>`subfolder`\</ib:inline-code\> is true) is optional - if the expression never queries the value of the \<ib:inline-code\>`subfolder`\</ib:inline-code\> value, Opus will simply recurse into all subfolders and won't call you in that mode again.
